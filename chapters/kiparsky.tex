...
the syntax, and that the verb-like case assignment of transitive agent nominalizations is due
...
The suffix \form{-ing} was shown to serve both as the gerund formative and as one of the
...
and which may be an expletive (\cref{gernom}{h}). In (\cref{gernom}{g}), \form{reading} refers to an event and \form{her} to its agent, the reader. In the derived nominal, \form{her} could also be a
sponsor, organizer, or some other participant of a reading event not necessarily identical with
the reader (Kratzer 1996, 2004), and \form{reading} could also mean `manner of reading',
`interpretation'.  Without an \form{of} complement, derived nominals are also interpretable
...
(\form{-ing\urf{N}}, \form{-ion}, etc.)  takes a bare VP complement. Because it has no Asp
...
provides no Spec-IP or Spec-TP for them.\footnote{Expletive \form{there}, which likewise
  ...
  it in the specifier of v.  Like expletive \form{it}, \form{there} does not appear in
...
affixation of \form{-ing} to the phrasal projection AspP.\footnote{A similar earlier proposal
  is \citet{yoon1996}.}  A lexicalist perspective rules out affixation to phrases.  It dictates an
entirely different kind of derivation, in which the gerund suffix \form{-ing} is added to
verbs in the morphology to build words (e.g.\ \form{reading}), which are then inserted in
...
The key idea is that gerunds are participles, and that participial suffixes, \form{-ing}
...
\begin{exe}
\ex\label{part}
	\ea English \form{-ing} participle
...
case-needing Infl \form{-ing} is suffixed to V in the morphology to form a participle, and
...
\ex\label{noo}
	\begin{xlist}
	\ex[*]{The/a/each compiling the corpus took over a year.}
	\ex[*]{Both/every compiling corporas took over a year.}
	\ex[*]{His two compilings corpora each took over a year.}
	\ex[*]{His careful compiling the corpus was a turning point.}
	\ex[*]{His editing texts that is funded will take a year.}
	\ex[*]{His compiling corpora's results were dramatic.}
	\end{xlist}
\end{exe}
...
\footnotetext{Cf.\ \textit{It's no good this sort of game.} (Dickens, \textit{Our Mutual
    Friend)}}  This does not mean that all transitive verbs take gerund complements.
...
the overt subject of an infinitive requires a Case-assigning \form{for}.  Crucially, they
...
\form{that-}clauses and other CP clauses must extrapose.\footnote{However, the case marking
  of participles in inflected languages could be considered as a kind of complementizer, as
  conjectured for the inherent case affix \form{-n} on Finnish gerunds in section
  \ref{finnsection}.}
...
with true structural subjects.  Their Case is inherently marked by the oblique suffix \form{-n},
...
Unlike English gerunds, Finnish gerunds are
never external arguments.  Thus they can be objects of transitive verbs such as ``say'',
``think'', ``want'', ``prove'', ``remember'' and ``hear'', and subjects of presentational
intransitives like ``appear'' and ``become evident'', but they cannot be subjects of such
predicates as ``be obvious'', ``prove'', and ``mean''.\footnote{In the glosses,
  \textsc{acc\trf{gen}} and \textsc{acc\trf{nom}} both refer to morphosyntactic Accusative
  structural case. The subscripts show three different morphological case realizations of this
  morphosyntactic Case.  They will become important shortly, but for now the reader may ignore
  them.}
\begin{exe}
\ex\label{unacc}
	\begin{xlist}
	...
	\end{xlist}
\end{exe}
This distribution suggests that the ending \form{-n} that participles bear in their gerundial
function, glossed ``\textsc{gen}'' in \rf{unacc}, marks an object Case that is compatible with
internal arguments but not with external arguments.  Historically, it is probably the old
...
pronoun-headed finite \form{se että-} (\form{it that-}) clauses  \rfa{luule}{d}.
\begin{exe}
\ex\label{luule}
...
picks out the agent of the shooting event, whereas the specifier \form{Matin} of the derived
...
tenseless nonfinite complements such as the third infinitive \rfa{lbc}{b} and the second
...
of accusative Case morphology.  Descriptively, Finnish morphosyntactic Case is realized as
morphological case as follows.\footnote{For details see \citealt{kiparsky2001}; a sophisticated OT
  treatment of the variation is developed by \citet{anttila2016}.}
\begin{exe}
\ex\label{genpart}
	\ea The subject of a participial clause is always Genitive.  
	\ex The object of a participial clause can be morphosyntactic Accusative or Partitive.
...
Accusative is realized by three morphological cases:
\begin{exe}
\ex\label{case}
	\ea as morphological accusative on personal pronouns, 
	\ex otherwise as morphological genitive if the object is plural, or if the clause has a
  subject with structural case (this last condition is called \textsc{Jahnsson's Rule}),
	\ex otherwise as morphological nominative. 
	\z
\end{exe}
Clause types that lack subjects with structural case for purposes of Jahnsson's Rule include
...
participle, being passive, is subjectless (Jahnsson's Rule, \rfa{case}{b}).\footnote{The
  ...
  Accusative (realized as nominative) is likely to be interpreted factively.}\todo{Hakulinen et al references are missing from the bibliography; please supply.} In
...
\form{Matin}.  Therefore the subject has structural Case.
\ea\label{casex}
...
\label{accing}
The English ``Acc-\form{ing}'' construction differs in many ways from the
``Poss-\form{ing}'' gerund considered here so far.  I have no serious analysis of it to
...
Acc-Inf gerunds allow extraction,  like Acc-Inf complements and  unlike Poss-\form{ing} gerunds:
\begin{exe}
\ex
...
a null preposition or complementizer, an analog of the overt \form{for} of \form{for-to}
infinitives. 
...
\subsection{Vedic agent nouns}

Vedic and Pāṇinian Sanskrit has a large number of agent noun suffixes, which fall into two
clearly demarcated types.  A minimal pair that highlights the contrast are the two agent noun
types in accented \form{-tár-\urf{N}} and preaccenting \form{′-tar-\urf{V}}.\footnote{Their
  Indo-European provenance is guaranteed by Greek and Avestan cognates \citep{lowe2014}.  The
  following exposition of their contrasting semantics, morphophology, and syntax draws on the
...
\form{′-tar-\urf{V}} (boldfaced in \rf{agex}) regularly assign structural case to their
objects and, can get certain aspectual adverb  modifiers, such as \textit{punaḥ} `again' in
\rfa{agex}{b}.\footnote{Other agent nouns with verbal properties are attested in early Vedic
...
feminines, and can be compounded.\footnote{E.g.\ \form{kṣirá-hotar-} `milk-offerer' (ŚBr.),
...
agentive.\footnote{Thus, the following roots do \textit{not} take either \form{-tár-\urf{N}}
  and \form{′-tar-\urf{V}} or any other agent suffixes for that matter: \form{as} `be',
...
The agent nominalizers \form{′-tar-\urf{V}} and \form{-tár-\urf{N}} form a
...
Vedic data.  The unmarked member of the opposition is \mbox{\form{-tár-\urf{N}}}, which
simply denotes agency (like English \form{-er}).  The marked member \form{′-tar-\urf{V}}
...
structural case and allowing adverbial modification, should be structurally high, while
\form{-tár-\urf{N}} nominalizations, which have strictly nominal properties, should be
structurally low.  Neither of these is the case.  In fact, as far as the case and adverb
...
bare verbal root directly, without any other intervening suffix; it cannot be added to compound
or prefixed bases.  \form{-tár-\urf{N}}, on the contrary, can be separated from the root by
verb-to-verb suffixes commonly analyzed as Voice/v heads (causative, denominative, intensive,
desiderative).  It is affixed to the whole verb base, including the extended root plus any
...
is high comes from tmesis, the splitting of prefixes from stems.  Prefixes can be separated
...
difference: low suffixes such as the agent suffix \form{′-tar-\urf{V}} are added directly to
the root to form a noun, which can then be composed with a prefix (see \rfa{baree}{a}), while high
suffixes such as the agent suffix \form{-tár-\urf{N}} are added to the entire verb, which may
...
roots require high \form{-tár-\urf{N}}, because the right-branching constituent structure
\rfa{baree}{a} would require them to combine with nouns.\footnote{Many examples are given in
  ...
  \form{hmm}' cannot be compounded with nouns.  It can only combine with the root
  \form{kṛ} `do', `make'.  The agent noun from \form{hiṁ-kṛ-} `to make the
...
with roots, only with nouns, require the right-branching constituent structure \rfa{baree}{a},
...
\form{′-tar-\urf{V}}.\footnote{Again we must make do with a couple of examples.  There is no
  compound verb such as \form{*para-apara-i-} `to go far and near' from which
  \form{párāpara-etar-} `one who goes far and near' might be derived.  In fact
  \form{párāpara-} `far and near' is never compounded with verbs.  Instead, the agent noun is
  a nominal compound formed from \form{para-apara-} `far and near' plus \form{e-tár-} `goer' (←
...
\rf{baree}.  But Distributed Morphology is a resourceful theory that makes available various
movement operations that cause mismatches between morphology and syntax. So could the
...
actual position?  The answer is negative on both counts.
   ...
   The way morphologically low suffixes such as the agent suffix \form{′-tar-\urf{V}} could
   be syntactically high for purposes of the FNT is by DM's \textsc{Lowering} operation, which
   applies before Vocabulary insertion to adjoin a head to the head of its complement \citep{embick2001}: \begin{exe}
\ex
...
\end{exe}
In English, Lowering of T is assumed to adjoin T to v (Embick \& Marantz 2008).
...
have any properties of a subject nominalizer – it is a true agent nominalizer, as we showed
above.
...
agent nouns made with  \form{-er}, and Finnish agent nouns made with
\form{-ja} have no inherent aspect: a \textit{driver} can be a habitual or professional
...
\form{-ja} is not an agent nominalizer, but a subject nominalizer, which is to say a high
...
\ex
  ...
  sing', cf. \form{lauja-ja} `singer', from \form{laula-} `to sing'.
	...
	\ex Inchoative + causative: \form{selv-en-tä-jä} `clarifier', ←  \form{selv-en-tä-}
...
\form{jälleen} `again' appears with an agent noun.
\begin{exe}
\ex\label{cann2} 
...
The lexicalist alternative, however, holds up.  Like English \form{-er}, nouns in
\form{-ja} are morphologically incompatible with overt Aspect or Voice morphology, and
...
\end{exe} 
Since \form{-ja} has no Aspect features and no verbal properties, the fact that it
...
\subsection{The Sakha agent nominalizer \form{-AAccY}}
...
Our approach predicts the transitivity of Sakha agent nouns in \form{-aaccy} out of the box.
The reason is that they have an aspect feature.  Agent nouns in Sakha \form{-aaccy} denote
...
\form{-tár-\urf{N}}, Finnish \form{-ja}, and English \form{-er}.  This accounts fully for
